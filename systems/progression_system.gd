...
func check_loss(run_state: RunState) -> bool:
	...
	var catastrophe: float = HeatSystem.catastrophe_ratio(HeatSystem.work_tier(run_state))
	if run_state.flags.get("fire_risk", false) and float(run_state.compute.get("heat", 0.0)) >= capacity * catastrophe:

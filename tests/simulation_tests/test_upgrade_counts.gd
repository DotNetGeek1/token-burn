...
func _test_major_machines_are_capped_by_floor_space() -> void:
	var shop: Dictionary = _shop("garage")
	var rack: UpgradeDefinition = ContentDatabase.get_upgrade("upgrade.gpu_rack")
	assert_true(_buy(shop, "upgrade.gpu_rack"), "First GPU rack is bought")
	assert_true(_buy(shop, "upgrade.gpu_rack"), "Second GPU rack is bought")
	assert_true(
		UpgradeSystem.hardware_slots_used(shop["state"], ContentDatabase)
		< UpgradeSystem.hardware_slots_total(shop["state"], ContentDatabase),
		"A slot remains after two racks"
	)
	assert_true(_buy(shop, "upgrade.gpu_rack"), "A third GPU rack is bought while a slot remains")
	assert_false(UpgradeSystem.is_maxed(shop["state"], rack), "The rack model is not level-capped")
	assert_true(
		UpgradeSystem.hardware_space_full(shop["state"], rack, ContentDatabase),
		"The garage floor is now full"
	)
	assert_false(_buy(shop, "upgrade.gpu_rack"), "The next copy is refused for floor space")
	assert_false(
